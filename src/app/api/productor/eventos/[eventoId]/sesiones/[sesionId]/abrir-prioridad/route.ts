...
async function handler(request: NextRequest, { params }: { params: { eventoId: string; sesionId: string } }) {
  ...
  const response = await fetch(`${API_URL}/productor/eventos/${params.eventoId}/sesiones/${params.sesionId}/abrir-prioridad`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to open priority' },
